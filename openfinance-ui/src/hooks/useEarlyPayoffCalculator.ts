...
function computeIRA(
  ...
  const cap1 =
    cfg.iraMonthsInterestCap > 0
      ? multiply(multiply(lumpSumAmount, monthlyRate), cfg.iraMonthsInterestCap)
      : Infinity;
  const cap2 =
    cfg.iraCapitalPercentCap > 0 ? multiply(balanceBefore, cfg.iraCapitalPercentCap) : Infinity;
...
function buildScenario(
  ...
  const interestSaved = baseScenario
    ? Math.max(0, subtract(baseScenario.totalInterest, totalInterest))
    : 0;
...
export function useEarlyPayoffCalculator(cfg: EarlyPayoffCountryConfig) {
  ...
  const calculate = useCallback(() => {
    ...
    const {
      rows: rdRows,
      baseMonthlyPayment: rdBase,
      finalMonthlyPayment: rdFinal,
    } = simulate(
      loanBalance,
      monthlyRate,
      totalRemainingMonths,
    ...
    const {
      rows: rpRows,
      baseMonthlyPayment: rpBase,
      finalMonthlyPayment: rpFinal,
    } = simulate(
      loanBalance,
      monthlyRate,
      totalRemainingMonths,
