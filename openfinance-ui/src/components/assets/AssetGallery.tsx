/**
 * Display-only gallery of an asset's image attachments.
...
export function AssetGallery({ assetId }: AssetGalleryProps) {
  const { t } = useTranslation('assets');
  const { images, isLoading } = useEntityImages(AttachmentEntityType.ASSET, assetId);
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-text-tertiary">{t('detail.gallery.hint')}</p>
    </div>
  );
}
