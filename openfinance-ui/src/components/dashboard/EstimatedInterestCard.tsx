...
export default function EstimatedInterestCard({ summary, period }: EstimatedInterestCardProps) {
  ...
  } = useSecondaryConversion(summary.currency || DEFAULT_CURRENCY);
  ...
  const displayEarned =
    filterType === 'ALL'
      ? summary.totalEarned
      : filteredAccounts.reduce((sum, acc) => sum + (acc.interestEarned || 0), 0);
  const displayProjected =
    filterType === 'ALL'
      ? summary.totalProjected
      : filteredAccounts.reduce((sum, acc) => sum + (acc.projectedInterest || 0), 0);
          ...
          <div className="w-36">
            ...
            >
              ...
              <option value="LIABILITIES">
                {t('estimatedInterest.filter.liabilities', 'Liabilities')}
              </option>
            </SimpleSelect>
          </div>
      ...
      <div className="space-y-6 flex-grow flex flex-col justify-between">
        ...
        <div className="text-center p-4 bg-surface-elevated rounded-lg border border-border">
          ...
          <div className="mt-2 flex items-center justify-center text-xs text-text-secondary gap-1">
            ...
            <span>
              {t('estimatedInterest.projectedNextYear')}:{' '}
