...
class BudgetControllerTest {
    ...
    void shouldGetAllBudgetsSuccessfully() throws Exception {
        ...
        when(budgetService.getBudgetsByUser(eq(USER_ID), any())).thenReturn(budgets);
    ...
    void shouldReturn401WhenNotAuthenticatedForGetAll() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/v1/budgets").header(ENCRYPTION_KEY_HEADER, testEncryptionKey))
                .andExpect(status().isUnauthorized());
    ...
    void shouldUpdateBudgetSuccessfully() throws Exception {
        ...
        when(budgetService.updateBudget(
                        eq(BUDGET_ID), any(BudgetRequest.class), eq(USER_ID), any()))
        ...
        verify(budgetService)
                .updateBudget(eq(BUDGET_ID), any(BudgetRequest.class), eq(USER_ID), any());
    }
    ...
    void shouldReturn404WhenBudgetNotFoundForDelete() throws Exception {
        ...
        doThrow(new BudgetNotFoundException("Budget not found with id: 999 for user: " + USER_ID))
                .when(budgetService)
                .deleteBudget(eq(999L), eq(USER_ID), any());
    ...
    void shouldUseDefaultPeriodWhenNotSpecified() throws Exception {
        ...
        when(budgetService.getAllBudgetsSummary(eq(USER_ID), any())).thenReturn(summaryResponse);
