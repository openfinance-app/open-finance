 ...
 *
 * <p>
 * These tests simulate multiple threads issuing requests at the same time to
 * verify that the
 * application handles concurrent access without deadlocks, excessive latency,
 * or data corruption.
 *
 * <p>
 * <b>Thresholds:</b>
 *
 * <ul>
 * <li>10 concurrent reads (accounts list): all succeed, total wall-time &lt; 5
 * s
 ...
 *
 * <p>
 * Note: MockMvc dispatches on the calling thread, so the executor threads each
 * create their own
 * MockMvc request. The shared Spring application context is thread-safe for
 * read operations; write
 * tests are kept at a lower concurrency to avoid H2 connection-pool exhaustion
 * (pool size = 5 in
 * {@code TestDatabaseConfig}).
 *
 * <p>
 * Requirement: TASK-13.1.6 (Performance tests — concurrent load)
 */
@SpringBootTest
...
class ConcurrentRequestPerformanceTest {
        ...
        /**
         * Sets up a test user and authentication token. Note: NOT annotated with
         * {@code @Transactional}
         ...
         *
         * <p>
         * Requirement: TASK-13.1.6 (Concurrent reads)
         */
        @Test
         ...
         *
         * <p>
         * A single warm-up request is issued first so that the net-worth snapshot for
         * today already
         * exists in the database before concurrent threads start. Without the warm-up,
         * all threads
         * would race to INSERT the same (user_id, snapshot_date) row, causing
         * unique-constraint
         * violations (HTTP 500).
         *
         * <p>
         * Requirement: TASK-13.1.6 (Concurrent dashboard reads)
         */
        @Test
        ...
        void concurrentDashboardReadsShouldAllSucceed() throws Exception {
                ...
                // Warm-up: seed today's net-worth snapshot so concurrent threads only UPDATE,
                // not INSERT
                mockMvc.perform(
                                get("/api/v1/dashboard")
                                                .header("Authorization", "Bearer " + capturedToken)
                                                .header("X-Encryption-Session", capturedEncKey))
                                .andExpect(status().isOk());
         ...
         *
         * <p>
         * Uses a lower concurrency than reads to stay within the H2 connection-pool
         * size configured
         * in {@code TestDatabaseConfig} (pool size = 5).
         *
         * <p>
         * Requirement: TASK-13.1.6 (Concurrent writes)
         */
        @Test
