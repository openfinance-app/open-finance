...
class PayeeServiceTest {
    ...
    void shouldNotFetchLogoWhenFindOrCreateFindsExistingPayee() {
        // Arrange
        when(payeeRepository.findAllByUser(USER_ID)).thenReturn(List.of(existingPayee));
    ...
    void shouldNotFetchLogoWhenFindOrCreateReactivatesInactivePayee() {
        ...
        Payee inactivePayee =
                Payee.builder()
                        ...
                        .build();
        when(payeeRepository.findAllByUser(USER_ID)).thenReturn(List.of(inactivePayee));
