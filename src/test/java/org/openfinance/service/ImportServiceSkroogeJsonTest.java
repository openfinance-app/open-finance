...
class ImportServiceSkroogeJsonTest {
    ...
    void shouldParseJsonImportsAndPreserveMetadata() throws Exception {
        ...
        List<ImportedTransaction> transactions = List.of(ImportedTransaction.builder()
        ...
        SkroogeImportParseResult parseResult = SkroogeImportParseResult.builder()
        ...
        when(fileStorageService.getFileContent(UPLOAD_ID)).thenReturn(new ByteArrayInputStream("{}".getBytes()));
        when(skroogeJsonParser.parseFile(any(InputStream.class), eq("my_export.json"))).thenReturn(parseResult);
    ...
    void shouldConfirmSkroogeJsonImportsWithTransfersAndSplits() throws Exception {
        ...
        Institution institution = Institution.builder().id(301L).name("Demo Bank").isSystem(false).build();
        ...
        when(accountService.createAccount(eq(USER_ID), any(), eq(encryptionKey)))
                .thenReturn(AccountResponse.builder().id(101L).build(), AccountResponse.builder().id(102L).build());
        ...
        when(categoryRepository.findByUserId(USER_ID)).thenAnswer(invocation -> new ArrayList<>(storedCategories));
        ...
        verify(transactionService, times(1)).createTransfer(eq(USER_ID), any(TransactionRequest.class),
                eq(encryptionKey));
        verify(transactionService, times(1)).syncTransactionFts(any(Transaction.class), eq("Local Market"),
                eq("Weekly groceries"));
        verify(accountService, times(2)).recalculateBalance(anyLong(), eq(USER_ID));
        verify(netWorthRepository, times(1)).deleteByUserIdAndSnapshotDateBefore(eq(USER_ID),
                eq(LocalDate.of(2024, 1, 12)));
    }

    @Test
    @DisplayName("Should create accounts without institution link when Skrooge institution name is blank")
    void shouldCreateAccountsWithoutInstitutionLinkWhenSkroogeInstitutionNameIsBlank() throws Exception {
        SkroogeImportMetadata metadata = SkroogeImportMetadata.builder()
                .institutions(List.of(SkroogeImportMetadata.SkroogeInstitution.builder()
                        .sourceId(99L)
                        .name("   ")
                        .build()))
                .accounts(List.of(SkroogeImportMetadata.SkroogeAccount.builder()
                        .sourceId(30L)
        ...
        ImportedTransaction transaction = ImportedTransaction.builder()
                .transactionDate(LocalDate.of(2024, 1, 10))
                .payee("Cash Deposit")
        ...
        verify(accountService).createAccount(eq(USER_ID), accountRequestCaptor.capture(), eq(encryptionKey));
    ...
    void shouldTruncateLongSkroogePayeesAndMemosToTransactionLimits() throws Exception {
        ...
        ImportedTransaction longTextTransaction = ImportedTransaction.builder()
        ...
        when(accountService.createAccount(eq(USER_ID), any(), eq(encryptionKey)))
                .thenReturn(AccountResponse.builder().id(101L).build(), AccountResponse.builder().id(102L).build());
        when(accountRepository.findById(101L)).thenReturn(Optional.of(checkingAccount));
        when(accountRepository.findById(102L)).thenReturn(Optional.of(Account.builder()
                .id(102L)
                .userId(USER_ID)
                .name("encrypted-savings")
                .currency("EUR")
                ...
                .build()));
        when(categoryRepository.findByUserId(USER_ID)).thenReturn(new ArrayList<>());
        when(transactionRepository.save(argThat(transaction -> transaction.getId() == null))).thenAnswer(invocation -> {
            Transaction saved = invocation.getArgument(0);
            saved.setId(801L);
            return saved;
        });
    ...
    void shouldSkipTinySkroogeAmountsWithoutFailingImportSession() throws Exception {
        SkroogeImportMetadata metadata = buildMetadataWithoutCategories();
        ImportedTransaction tinyAmountTransaction = ImportedTransaction.builder()
    ...
    private ImportSession buildSession(SkroogeImportMetadata metadata, List<ImportedTransaction> transactions)
            throws Exception {
        Map<String, Object> sessionMetadata = new HashMap<>();
        sessionMetadata.put("transactions", transactions);
        sessionMetadata.put("count", transactions.size());
    ...
    private List<ImportedTransaction> buildImportedTransactions() {
        ImportedTransaction splitExpense = ImportedTransaction.builder()
                .transactionDate(LocalDate.of(2024, 1, 10))
                .payee("Local Market")
                .amount(new BigDecimal("-45.00"))
        ...
        ImportedTransaction transferOut = ImportedTransaction.builder()
        ...
        ImportedTransaction transferIn = ImportedTransaction.builder()
                .transactionDate(LocalDate.of(2024, 1, 12))
                .payee("Transfer Desk")
                .amount(new BigDecimal("200.00"))
