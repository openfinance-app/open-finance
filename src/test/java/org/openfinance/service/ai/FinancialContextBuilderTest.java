...
/**
 * Unit tests for FinancialContextBuilder Task 11.1.7b: Write
 * FinancialContextBuilder unit tests
 *
 * <p>
 * Tests cover: - buildContext with full financial data - buildContext with
 * empty data -
 * buildMinimalContext - Decryption of encrypted fields - Context formatting and
 * sections
 */
...
class FinancialContextBuilderTest {
        ...
        class BuildContextTests {
                ...
                void shouldBuildContextWithAllData() {
                        ...
                        List<Asset> assets = Arrays.asList(createAsset(1L, "AAPL Stock", AssetType.STOCK, "10000.00"));
                        when(assetRepository.findByUserId(userId)).thenReturn(assets);

                        List<Liability> liabilities = Arrays.asList(createLiability(1L, "Credit Card", "1500.00"));
                ...
                void shouldHandleDecryptionFailures() {
                        ...
                        List<Account> accounts = Arrays
                                        .asList(createAccount(1L, "Checking", AccountType.CHECKING, "2500.00"));
                        when(accountRepository.findByUserIdAndIsActive(userId, true)).thenReturn(accounts);
                        when(netWorthService.calculateTotalAssets(eq(userId), eq("USD")))
                                        .thenReturn(BigDecimal.ZERO);
                        when(netWorthService.calculateTotalLiabilities(
        ...
        class BuildMinimalContextTests {
                ...
                void shouldBuildMinimalContext() {
                        ...
                        verify(netWorthService)
                                        .calculateTotalAssets(eq(userId), eq("USD"));
                        verify(netWorthService)
                                        .calculateTotalLiabilities(eq(userId), eq("USD"));
                        verify(accountRepository, times(1)).findByUserIdAndIsActive(userId, true);
                }
        }
