 ...
 * manually) since
 * the previous run. For each newly created transaction it applies the
 * rule-based statistical
 * algorithms implemented in {@link UnusualTransactionDetectionService} and
 * persists any anomalies
 * as {@code UNUSUAL_TRANSACTION} {@link org.openfinance.entity.Insight}
 * records, which are surfaced
 * to the user through the existing insight/notification pipeline.
 *
 * <p>
 * <strong>Default Schedule:</strong> Daily at 01:00 AM — intentionally one hour
 * after midnight
 * so that the recurring-transaction scheduler (00:00) and the net-worth
 * snapshot scheduler (00:05)
 ...
 * {@value #LOOKBACK_HOURS} hours from
 * the current run time so that a brief outage or restart does not cause
 * transactions to be silently
 * skipped.
 *
...
public class UnusualTransactionDetectionScheduler implements ApplicationRunner {
    ...
    /**
     * How many hours back from "now" to look for newly created transactions. Set to
     * 25 h so a brief
