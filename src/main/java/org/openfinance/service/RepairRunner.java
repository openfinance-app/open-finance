...
/**
 * Repair runner to detect and re-encrypt plaintext/malformed ciphertext stored
 * in the DB. Disabled
 * by default; enable with property `app.repair.enabled=true`.
 *
 * <p>
 * This is intentionally conservative: by default it runs in "dry-run" mode and
 * only logs
 * candidates. If you enable it with `app.repair.execute=true` it will perform
 * in-place
 * re-encryption for the demo user using `app.repair.demo-master-password` to
 * derive the key.
 */
@Component
...
public class RepairRunner implements CommandLineRunner {
        ...
        private boolean looksLikePlaintext(String value) {
                if (value == null)
                        return false;
                if (value.length() < 24)
                        return true; // too short for IV+ciphertext
                if (!value.matches("^[A-Za-z0-9+/=]+$"))
                        return true; // contains non-base64 chars
                return false;
        }
        ...
        private void repairRecurringTransactions(Long userId, SecretKey key) {
                List<org.openfinance.entity.RecurringTransaction> list = recurringTransactionRepository
                                .findByUserId(userId);
                for (var r : list) {
        ...
        private void repairRealEstate(Long userId, SecretKey key) {
                ...
                for (var p : list) {
                        ...
                        maybeReencryptAndSave(
                                        ...
                                        key);
                        if (repairExecute)
                                realEstateRepository.save(p);
                }
        }

        private void repairLiabilities(Long userId, SecretKey key) {
                List<org.openfinance.entity.Liability> list = liabilityRepository
