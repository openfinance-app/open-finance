...
public class TransactionService {
         ...
         */
        @Caching(evict = {
                        @CacheEvict(value = { "dashboardSummary", "accountSummaries",
                                        "netWorthSummary" }, key = "#userId"),
        ...
        public TransactionResponse createTransaction(
                        Long userId, TransactionRequest request) {
                if (userId == null) {
                ...
                if (request.getCategoryId() == null
                        ...
                        var payee = payeeRepository.findAll().stream()
                                        .filter(p -> p.getName() != null
                                                        && p.getName().equalsIgnoreCase(request.getPayee().trim()))
                ...
                if (request.getSplits() != null && !request.getSplits().isEmpty()) {
                        transactionSplitService.saveSplits(
                                        savedTransaction.getId(), request.getSplits());
                }
        ...
        public TransactionResponse createTransfer(
                ...
                if (!sourceCurrency.equalsIgnoreCase(destCurrency)) {
                        try {
                                destAmount = exchangeRateService.convert(
         ...
         * INCOME) in a
         * single atomic transaction. Both transactions share the same transferId and
         * will be updated
        ...
        public TransactionResponse updateTransfer(
                ...
                Account oldSourceAccount = accountRepository
                ...
                Account oldDestAccount = accountRepository
                ...
                Account newDestAccount = accountRepository
                                .findByIdAndUserId(request.getToAccountId(), userId)
                                .orElseThrow(
                                                () -> AccountNotFoundException.byIdAndUser(
                                                                request.getToAccountId(), userId));
        ...
        public TransactionResponse updateTransaction(
                ...
                transactionSplitService.saveSplits(
                                updatedTransaction.getId(), request.getSplits());
         ...
         * matches on
         * encrypted data, results may be limited. Consider decrypting all transactions
         * client-side for
        ...
        public org.springframework.data.domain.Page<TransactionResponse> searchTransactions(
                ...
                if (pageable.getSort().getOrderFor("id") == null) {
                        ...
                        org.springframework.data.domain.Sort.Direction idDir = (dateOrder != null
                                        && dateOrder.isAscending())
                ...
                return transactionPage.map(
                                transaction -> toResponseWithDecryption(transaction));
        }
        ...
        /**
         * Resolve or create a Payee entity and set payeeId on the transaction. If the
         * transaction's
        ...
        /**
         * Look up the Currency entity by the transaction's currency code and set
         * currencyId. If the
         * currency code is not found in the currencies table, currencyId is set to
         * null.
         */
         ...
         */
        /**
         * Returns the value as-is — JPA converter already handles decryption.
        ...
        public TransactionResponse toResponseWithDecryption(
                ...
                if (transaction.getId() != null) {
                        List<TransactionSplitResponse> splits = transactionSplitService.getSplitsForTransaction(
                                        transaction.getId());
         ...
         * the
         * transaction {@code amount} to the base currency using
        ...
        public List<TransactionResponse> getTransactionsByTag(
                ...
                List<Transaction> matchingTransactions = transactions.stream()
                                .filter(t -> t.getTags() != null && !t.getTags().isBlank())
                                .filter(
                                                t -> java.util.Arrays.stream(t.getTags().split(","))
                                                                .map(String::trim)
         ...
         *
         * <p>
         * Results are ordered by transaction date descending (newest first). Sensitive
         * fields are
        ...
        /**
         * Indexes a transaction's searchable fields as blind-index tokens.
         * Uses HMAC-based tokens via SearchTokenService for privacy-preserving search.
         */
        private void indexTransactionInFts(
                ...
                try {
                        ...
                        if (key == null) {
                                log.debug("No encryption key in context — skipping search token indexing for tx {}",
                                                transactionId);
                                return;
                        }
                        SecretKey searchKey = searchTokenService.deriveSearchKey(key);
                        searchTokenService.indexEntity(userId, "TRANSACTION", transactionId,
                                        java.util.List.of(
                        ...
                        log.warn("Failed to index transaction {} search tokens: {}", transactionId, e.getMessage());
                }
        }

        /**
         * Public entry point for indexing a transaction entity into the search token
         * table.
         * Used by services that save transactions directly (e.g. ImportService).
         */
        ...
        private void removeTransactionFromFts(Long transactionId) {
                try {
                        ...
                        log.warn("Failed to remove search tokens for transaction {}: {}", transactionId,
                                        e.getMessage());
                }
        }
         ...
         * targetDate T
         * by taking the current account balance and reversing every transaction whose
         * {@code date > T}.
         * So any transaction at date D is part of the reversal set for every snapshot
         * where {@code
         * snapshotDate < D}. When D changes (create, update, delete), those snapshots
         * are stale and
         * must be recomputed.
         *
         * <p>
         * After deletion the existing auto-backfill logic in {@code
         * DashboardController.getNetWorthHistory()} rebuilds the missing snapshots on
         * the next chart
