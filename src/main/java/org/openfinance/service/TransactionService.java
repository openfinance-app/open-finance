...
public class TransactionService {
        ...
        public org.springframework.data.domain.Page<TransactionResponse> searchTransactions(
                ...
                if (criteria.getKeyword() != null && !criteria.getKeyword().trim().isEmpty()) {
                        try {
                                ...
                                matchedTransactionIds = jdbcTemplate.queryForList(sql, Long.class,
                                                criteria.getKeyword(), userId);
        ...
        private String decryptFieldOrReturnRaw(
                        String storedValue, SecretKey encryptionKey, Long transactionId, String fieldName) {
                ...
                try {
                        ...
                        if (looksLikePlaintext(storedValue)) {
                                log.debug(
                                                "Transaction {} {} appears to be stored as plaintext; using raw value",
                                                transactionId,
                                                fieldName);
                        } else {
                                log.warn(
                                                "Failed to decrypt transaction {} for id={}; using raw value",
                                                fieldName,
                                                transactionId,
                                                e);
                        }
        ...
        public TransactionResponse toResponseWithDecryption(
                ...
                if (transaction.getDescription() != null && !transaction.getDescription().isBlank()) {
                        decryptedDescription = decryptFieldOrReturnRaw(
                                        transaction.getDescription(),
                                        encryptionKey,
                                        transaction.getId(),
                                        "description");
                ...
                if (transaction.getNotes() != null && !transaction.getNotes().isBlank()) {
                        decryptedNotes = decryptFieldOrReturnRaw(
                                        transaction.getNotes(), encryptionKey, transaction.getId(), "notes");
                }
                                ...
                                .ifPresent(
                                                account -> {
                                                        if (account.getName() != null && !account.getName().isBlank()) {
                                                                try {
                                                                        String decryptedAccountName = encryptionService
                                                                                        .decrypt(
                                                                                                        account.getName(),
                                                                                                        encryptionKey);
                                                                        ...
                                                                        // Decryption failure for denormalized/optional
                                                                        // fields should
                                                                        // not
                                                                        // fail the entire request. Log and continue
                                                                        // with empty name.
                                                                        log.warn(
                                        ...
                                        .ifPresent(
                                                        toAccount -> {
                                                                if (toAccount.getName() != null
                                                                        ...
                                                                        try {
                                                                                String decryptedToAccountName = encryptionService
                                                                                                .decrypt(
                                                                                                                toAccount.getName(),
                                                                                                                encryptionKey);
                                                                                response.setToAccountName(
                                                                                                decryptedToAccountName);
                                                                        } catch (IllegalArgumentException
                                                                                        | IllegalStateException e) {
                                                                                log.warn(
                ...
                if (transaction.getCategoryId() != null) {
                        categoryRepository
                                        ...
                                        .ifPresent(
                                                        category -> {
                                                                // Decrypt category name only if it's not a system
                                                                // category
                                                                String categoryName;
        ...
        /**
         * Indexes a transaction in the FTS table for full-text search. Replaces any
         * existing FTS entry
         ...
         * into the FTS
         * table. Used by services that save transactions directly (e.g. ImportService)
         * and bypass the
