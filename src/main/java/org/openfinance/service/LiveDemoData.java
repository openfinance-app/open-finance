...
/**
 * Seeds a complete, realistic live-demo dataset for the Open-Finance demo environment.
 *
 * <p>This bean is only instantiated when {@code application.live-demo.enabled=true} is set, so it
 * has no impact on production or test environments unless explicitly enabled.
 *
 * <p>The demo persona is <em>Sophie Martin</em>, a 34-year-old software engineer based in Paris,
 * France, with a diversified portfolio, two real-estate properties, realistic recurring expenses
 * and ~14 months of transaction history.
 *
 * <p>Credential defaults (override in application.yml or env vars if desired):
 *
 * <ul>
...
@ConditionalOnProperty(
        name = "application.live-demo.enabled",
        havingValue = "true",
...
public class LiveDemoData implements CommandLineRunner {
    ...
    /**
     * Returns accounts in a stable order so downstream methods can reference them by index without
     * decrypting names.
     *
     * <p>Index contract:
     *
    ...
    /**
     * Repairs the FTS index for the demo user if it contains empty entries. This handles existing
     * databases where demo transactions were seeded before the FTS sync was in place.
     */
            ...
            userRepository
                    ...
                    .ifPresent(
                            user -> {
                                ...
                                log.info(
                                        "Repairing FTS for {} demo transaction entries...",
                                        emptyFtsCount);
                                ...
                                // Load all demo transactions and re-populate FTS with decrypted
                                // content
                                ...
                                for (Transaction t : transactions) {
                                    try {
                                        ...
                                        log.warn(
                                                "Failed to repair FTS entry for transaction {}: {}",
                                                t.getId(),
                                                e.getMessage());
                                    }
                                }
                                log.info(
                                        "FTS repair completed — {} entries updated",
                                        transactions.size());
                            });
        } catch (Exception e) {
