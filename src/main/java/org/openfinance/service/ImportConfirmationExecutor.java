...
/**
 * Runs the (potentially long-running) import confirmation on a background
 * thread.
 *
 * <p>
 * This lives in a dedicated Spring bean so that the {@link Async} proxy
 * boundary is actually
 * crossed when {@link ImportService} triggers it. Calling an {@code @Async}
 * method from within the
 * same bean (self-invocation) bypasses the proxy and executes synchronously,
 * which is precisely the
 * behaviour we want to avoid here — the confirmation of large imports can take
 * well over a minute
...
public class ImportConfirmationExecutor {
    ...
    /**
     * Execute the import confirmation asynchronously. On success the session is
     * left in {@code
     * COMPLETED} status; on failure it is marked {@code FAILED} so the polling
     * client can surface an
     * error.
     *
