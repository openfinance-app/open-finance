...
public class NotificationService {
        ...
        private List<NotificationResponse> checkLowBalances(Long userId) {
                ...
                List<Account> lowBalanceAccounts = accountRepository.findByUserIdAndIsActive(userId, true).stream()
                                .filter(a -> a.getBalance() != null
                                                && a.getBalance().compareTo(LOW_BALANCE_THRESHOLD) < 0)
                                .toList();
        ...
        /**
         * Checks whether any exchange rates have been stored in the last 2 days. If no
         * exchange rate
         * was updated within that window the user is notified so they can trigger a
         * manual refresh.
         */
