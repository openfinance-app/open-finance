...
public class LiabilityService {
        ...
        public LiabilityResponse createLiability(
                        Long userId, LiabilityRequest request) {
                if (userId == null) {
                ...
                if (request.getInstitutionId() != null) {
                        org.openfinance.entity.Institution institution = institutionRepository
                                        .findById(request.getInstitutionId())
                                        .orElseThrow(
                                                        () -> new org.openfinance.exception.InstitutionNotFoundException(
        ...
        public LiabilityResponse updateLiability(
                ...
                LocalDate cutoff = (oldStartDate != null
         ...
         *
         * <p>
         * Returns all liabilities with decrypted data and calculated fields, ordered by
         * creation
        ...
        public List<LiabilityResponse> getLiabilitiesByType(
                        Long userId, LiabilityType type) {
        ...
        /**
         * Fields whose values are AES-256-encrypted in the database. Sorting or
         * searching on these
         * fields must be done in memory after decryption; they cannot be delegated to
         * the SQL layer.
         */
         ...
         *
         * <p>
         * When {@code search} is non-null <em>or</em> the requested sort field is one
         * of the
         * AES-256-encrypted columns, the method falls back to a full in-memory fetch,
         * decrypt, filter
         * and sort cycle before paginating manually. This avoids meaningless LIKE /
         * ORDER BY operations
        ...
        public Page<LiabilityResponse> getLiabilitiesWithFilters(
                ...
                if (hasSearch || sortOnEncrypted) {
                        ...
                        List<LiabilityResponse> responses = all.stream()
                                        .map(l -> toResponseWithDecryption(l))
                                        .collect(Collectors.toList());
                ...
                Page<Liability> liabilitiesPage = liabilityRepository.findByUserIdWithFilters(userId, type, pageable);
         ...
         *
         * <p>
         * Sums all current balances across all liabilities. Liabilities in different
         * currencies are
        ...
        public List<AmortizationScheduleEntry> calculateAmortizationSchedule(
                ...
                if (minimumPayment == null || minimumPayment.compareTo(BigDecimal.ZERO) <= 0) {
                        ...
                        if (principal != null
                                ...
                                if (totalMonths > 0) {
                                        BigDecimal mRate = interestRate.divide(
                                        ...
                                        if (mRate.compareTo(BigDecimal.ZERO) > 0) {
                                                try {
                                                        BigDecimal onePlusRPowN = mRate.add(BigDecimal.ONE)
                                                                        .pow((int) totalMonths);
                                                        ...
                                                        log.warn("Error calculating minimum payment: {}",
                                                                        e.getMessage());
                                                }
                                        } else {
                                                // 0% interest rate
                                                minimumPayment = principal.divide(
                                                                BigDecimal.valueOf(totalMonths), 2,
                                                                RoundingMode.HALF_UP);
                                        }
                                }
                        }
                ...
                while (remainingBalance.compareTo(BigDecimal.ZERO) > 0
                        ...
                        BigDecimal interestPortion = remainingBalance.multiply(monthlyRate).setScale(2,
                                        RoundingMode.HALF_UP);
        ...
        public BigDecimal calculateTotalInterest(
                ...
                List<AmortizationScheduleEntry> schedule = calculateAmortizationSchedule(liabilityId, userId);
        ...
        public LiabilityBreakdownResponse getLiabilityBreakdown(
                ...
                if (!schedule.isEmpty()) {
                        ...
                        if (interestRate != null && interestRate.compareTo(BigDecimal.ZERO) > 0) {
                                ...
                                // balance
                                BigDecimal avgBalance = principal
                                                .add(currentBalance)
                                                .divide(BigDecimal.valueOf(2), SCALE, RoundingMode.HALF_UP);
                                estimatedInterestPaid = avgBalance
                ...
                List<Transaction> linkedTransactions = transactionRepository.findByLiabilityIdAndUserId(liabilityId,
                                userId);
        ...
        private LiabilityResponse toResponseWithDecryption(
                ...
                if (decryptedInterestRate != null && decryptedInterestRate.compareTo(BigDecimal.ZERO) > 0) {
                        BigDecimal monthlyRate = decryptedInterestRate.divide(
                                        BigDecimal.valueOf(MONTHS_PER_YEAR * 100), SCALE, RoundingMode.HALF_UP);
                        monthlyInterestCost = decryptedCurrentBalance.multiply(monthlyRate).setScale(2,
                                        RoundingMode.HALF_UP);
                }
         ...
         * the {@code
         * currentBalance} to the base currency using {@link ExchangeRateService}. On
         * failure, falls
        ...
        private void populateConversionFields(
                ...
                String baseCurrency = user != null && user.getBaseCurrency() != null
                                && !user.getBaseCurrency().isBlank()
                ...
                if (!needsConversion || nativeBalance == null) {
                        ...
                        try {
                                BigDecimal rate = exchangeRateService.getExchangeRate(nativeCurrency, baseCurrency,
                                                null);
                                BigDecimal converted = exchangeRateService.convert(nativeBalance, nativeCurrency,
                                                baseCurrency);
                ...
                if (secCurrency != null
                                ...
                                && nativeBalance != null) {
                        try {
                                BigDecimal secRate = exchangeRateService.getExchangeRate(nativeCurrency, secCurrency,
                                                null);
                                BigDecimal secAmount = exchangeRateService.convert(nativeBalance, nativeCurrency,
                                                secCurrency);
        ...
        private void indexLiabilitySearchTokens(Liability liability, String name) {
                try {
                        ...
                        searchTokenService.indexEntity(liability.getUserId(), "LIABILITY", liability.getId(),
                                        java.util.List.<String[]>of(new String[] { "name", name }),
                                        searchKey);
                } catch (Exception e) {
                        log.warn("Failed to index liability {} search tokens: {}", liability.getId(), e.getMessage());
                }
        }
}
