...
public class DashboardService {
    ...
    public DashboardSummary getDashboardSummary(Long userId, SecretKey encryptionKey) {
        ...
        List<TransactionResponse> recentTransactions =
                getRecentTransactions(userId, 10, encryptionKey);
    ...
    public NetWorthSummary getNetWorthSummary(Long userId, SecretKey encryptionKey) {
        ...
        NetWorth currentNetWorth =
                netWorthService.saveNetWorthSnapshot(userId, today, encryptionKey, baseCurrency);
                                ...
                                transaction -> {
                                    ...
                                    if (transaction.getNotes() != null
                                        ...
                                        try {
                                            response.setNotes(
                                                    encryptionService.decrypt(
                                                            transaction.getNotes(), encryptionKey));
                                        } catch (IllegalArgumentException
                                                | IllegalStateException e) {
                                            log.warn(
    ...
    public List<AssetAllocation> getAssetAllocation(Long userId) {
        ...
        BigDecimal totalPortfolioValue =
                assets.stream().map(Asset::getTotalValue).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    private List<PortfolioPerformance> buildPortfolioPerformance(
        ...
        BigDecimal totalCost =
                assets.stream().map(Asset::getTotalCost).reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        for (Account account : accounts) {
            if (Boolean.TRUE.equals(account.getIsInterestEnabled())) {
                ...
                try {
                    ...
                    log.warn(
                            "Failed to decrypt account name for account id={}", account.getId(), e);
