...
public class DashboardService {
        ...
        /**
         * Days per month — used for period estimation in borrowing-capacity
         * calculations.
         */
         ...
         * change. If no
         * net worth snapshot exists for today, it calculates the current net worth
         * on-the-fly.
         *
         * <p>
         ...
         * {@code netWorthSummary} for
         * up to 15 minutes to avoid repeated snapshot writes and asset-valuation
         * queries. The cache is
         * evicted by {@link org.openfinance.service.NetWorthService} whenever new data
         * is saved.
         *
         ...
         * key. Accounts
         * are sorted by balance descending (highest balance first). Only active
         ...
         * and returns
         * them as a list of {@link TransactionResponse} DTOs. Sensitive fields
         * (description, notes,
         * account/category names) are decrypted using the provided
         * {@code encryptionKey}.
         *
         * <p>
         * <b>Performance Optimization (REQ-3.1):</b> To avoid the N+1 query problem,
         * this method
         * batch-loads all associated {@link Account} and {@link Category} entities in
         * two single
         * database round-trips using {@code findAllByIds}, rather than issuing a
         * separate query for
         * each transaction.
         *
         * @param userId        the ID of the user
        ...
        public Map<String, BigDecimal> getCashFlow(Long userId, int period) {
                ...
                List<Transaction> transactions = transactionRepository.findByUserIdAndDateBetween(userId, startDate,
                                endDate);
                ...
                BigDecimal income = transactions.stream()
                                .filter(
                                                t -> t.getType() == TransactionType.INCOME
                                                                && !t.getIsDeleted()
        ...
        /**
         * Calculates cash flow for the specified user between two explicit dates. Not
         * cached – used for
         ...
         * Only
         * non-deleted expense transactions are included. Results are sorted by amount
         * descending
        ...
        /**
         * Calculates spending breakdown by category between two explicit dates. Not
         * cached – used for
         * custom date range requests.
         */
         ...
         *
         * <p>
         * Groups INCOME transactions by category (or "Uncategorized") to produce income
         * source
         * nodes, and groups EXPENSE transactions by resolved category name to produce
         * expense category
        ...
        /**
         * Builds Cashflow Sankey diagram data between two explicit dates. Not cached –
         * used for custom
         * date range requests.
         */
        ...
        private CashflowSankeyDto buildCashflowSankey(
                ...
                User sankeyUser = userRepository
                                .findById(userId)
                                .orElseThrow(
                                                () -> new IllegalArgumentException("User not found: " + userId));
                ...
                List<CashflowSankeyDto.FlowNode> incomeSources = incomeByCategoryKey.entrySet().stream()
                                ...
                                .map(
                                                entry -> {
                                                        org.openfinance.entity.Category cat = resolveCategory
                                                                        .apply(entry.getKey());
         ...
         * portfolio
         * across different asset types (STOCK, CRYPTO, BOND, etc.). Each allocation
         * includes:
         *
         * <ul>
        ...
        /**
         * Calculates portfolio performance between two explicit dates. Not cached –
         * used for custom
         * date range requests.
         */
        ...
        private List<PortfolioPerformance> buildPortfolioPerformance(
                ...
                List<PortfolioPerformance.HistoricalDataPoint> sparklineData = netWorthHistory.stream()
                                ...
                                .map(
                                                nw -> PortfolioPerformance.HistoricalDataPoint.builder()
        ...
        public BorrowingCapacity getBorrowingCapacity(Long userId, int analysisPeriod) {
                ...
                BigDecimal monthsInPeriod = BigDecimal.valueOf(analysisPeriod).divide(DAYS_PER_MONTH, 2,
                                RoundingMode.HALF_UP);
                ...
                BigDecimal recommendedMaxBorrowing = maxDebtAt40Percent
                ...
                if (monthlyIncome.compareTo(BigDecimal.ZERO) == 0) {
                        financialHealthStatus = "INSUFFICIENT_DATA";
                                ...
                                businessRules.getDebtToIncome().getExcellentMaxPercent()) <= 0) {
                        financialHealthStatus = "EXCELLENT";
                } else if (debtToIncomeRatio.compareTo(businessRules.getDebtToIncome().getGoodMaxPercent()) <= 0) {
                        financialHealthStatus = "GOOD";
                } else if (debtToIncomeRatio.compareTo(businessRules.getDebtToIncome().getFairMaxPercent()) <= 0) {
        ...
        /**
         * Calculates borrowing capacity between two explicit dates. Not cached – used
         * for custom date
         * range requests.
         */
        public BorrowingCapacity getBorrowingCapacity(
                ...
                BigDecimal availableBorrowingCapacity = recommendedMaxBorrowing
                                ...
                                .multiply(
                                                BigDecimal.valueOf(
                                                                businessRules.getDebtToIncome()
                                                                                .getBorrowingTermYears()))
                                .setScale(2, RoundingMode.HALF_UP);
        ...
        public List<NetWorthAllocation> getNetWorthAllocation(Long userId) {
                ...
                for (Account account : accounts) {
                        ...
                        BigDecimal convertedBalance = convertToBase(account.getBalance(), account.getCurrency(),
                                        baseCurrency);
                ...
                for (Asset asset : assets) {
                        ...
                        BigDecimal convertedAssetValue = convertToBase(asset.getTotalValue(), asset.getCurrency(),
                                        baseCurrency);
                ...
                for (Liability liability : liabilities) {
                        ...
                        BigDecimal convertedLiabilityBalance = convertToBase(balance, liability.getCurrency(),
                                        baseCurrency);
                ...
                List<NetWorthAllocation> allocations = categoryMap.entrySet().stream()
                                .map(
                                                entry -> {
                                                        ...
                                                        BigDecimal percentage = grossAssets
                                ...
                                .filter(
                                                a -> a.getValue().abs().compareTo(BigDecimal.ZERO) > 0) // Exclude zero
                                // balances
                                .sorted(
                                                (a, b) -> b.getValue()
                                                                .abs()
        ...
        /**
         * Converts an amount from one currency to the user's base currency. Falls back
         * to the original
        ...
        public EstimatedInterestSummary getEstimatedInterestSummary(Long userId, String period) {
                ...
                for (Liability liability : liabilities) {
                        ...
                        try {
                                ...
                                BigDecimal periodInterest = annualInterest
         ...
         * total net
         * worth. Year range is determined from the user's earliest transaction date to
         * the latest
        ...
        public YearlyBalanceResponse getYearlyBalanceVariations(Long userId) {
                ...
                for (int i = 0; i < accounts.size(); i++) {
                        ...
                        Long instId = account.getInstitution() != null ? account.getInstitution().getId() : null;
                        if (instId == null) {
        ...
        /**
         * One month as an exact fraction of a year (1/12), computed in BigDecimal
         * (never double).
         */
        ...
        /**
         * {@code days} as an exact fraction of a 365-day year, computed in BigDecimal
         * (never double).
         */
