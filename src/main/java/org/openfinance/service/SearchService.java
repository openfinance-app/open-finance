 ...
 * FTS table stores
 * decrypted text for search functionality, but database file encryption
 * (SQLCipher) protects data
 * at rest.
 *
 * <p>
 * Task: TASK-12.4.2
...
public class SearchService {
        ...
        public GlobalSearchResponse globalSearch(
                        Long userId, String query, Integer limit) {
                ...
                CompletableFuture<List<SearchResultDto>> assetsFuture = CompletableFuture.supplyAsync(
                                () -> withEncryptionKey(encKey,
                                                () -> searchAssets(userId, query, effectiveLimit)));
        ...
        public GlobalSearchResponse advancedSearch(
                        Long userId, AdvancedSearchRequest request) {
                long startTime = System.currentTimeMillis();
                ...
                // Apply limits
                int effectiveLimit = (request.getLimit() != null
                                && request.getLimit() > 0
                ...
                if (entityTypesToSearch.contains(SearchResultType.TRANSACTION)) {
                        List<SearchResultDto> transactionResults = searchTransactionsWithFilters(userId, request,
                                        effectiveLimit);
                ...
                if (entityTypesToSearch.contains(SearchResultType.ACCOUNT)) {
                        List<SearchResultDto> accountResults = searchAccountsWithFilters(userId, request,
                                        effectiveLimit);
                ...
                if (entityTypesToSearch.contains(SearchResultType.ASSET)) {
                        List<SearchResultDto> assetResults = searchAssetsWithFilters(userId, request, effectiveLimit);
                ...
                if (entityTypesToSearch.contains(SearchResultType.REAL_ESTATE)) {
                        List<SearchResultDto> realEstateResults = searchRealEstate(userId, request.getQuery(),
                                        effectiveLimit);
                ...
                if (entityTypesToSearch.contains(SearchResultType.LIABILITY)) {
                        List<SearchResultDto> liabilityResults = searchLiabilities(userId, request.getQuery(),
                                        effectiveLimit);
                ...
                if (entityTypesToSearch.contains(SearchResultType.BUDGET)) {
                        List<SearchResultDto> budgetResults = searchBudgets(userId, request.getQuery(), effectiveLimit);
                ...
                if (entityTypesToSearch.contains(SearchResultType.RECURRING_TRANSACTION)) {
                        List<SearchResultDto> recurringResults = searchRecurringTransactions(
                                        userId, request.getQuery(), effectiveLimit);
                        allResults.addAll(recurringResults);
                }

                // Search categories
                if (entityTypesToSearch.contains(SearchResultType.CATEGORY)) {
                        List<SearchResultDto> categoryResults = searchCategories(userId, request.getQuery(),
                                        effectiveLimit);
                                                        ...
                                                        account -> {
                                                                try {
                                                                        ...
                                                                        String snippet = createSnippet(
                                                                                        name, description,
                                                                                        request.getQuery(), 150);

                                                                        return SearchResultDto.builder()
                                                                                        .resultType(SearchResultType.ACCOUNT)
                                                                                        .id(account.getId())
                                                                                        .title(name)
                                                                                        .subtitle(account.getType()
                                                                                                        .name())
                                                                                        .amount(account.getBalance())
                                                                                        .currency(account.getCurrency())
                                                                                        .icon(getAccountTypeIcon(account
                                                                                                        .getType()))
                                                                        ...
                                                                        log.error("Error processing account search result",
                                                                                        e);
                                                                        return null;
                                                                }
                                                        })
                                        ...
                                        .filter(
                                                        asset -> {
                                                                ...
                                                                if (request.getMinAmount() != null
                                                                        ...
                                                                        if (asset.getCurrentPrice().compareTo(
                                                                ...
                                                                if (request.getDateFrom() != null
                                                                        ...
                                                                        if (asset.getPurchaseDate().isBefore(
                                                                                        request.getDateFrom())) {
                                                                                return false;
                                                                        }
                                                                }
                                                                ...
                                                                String name = asset.getName() != null
                                                                                ? asset.getName()
                                                                                : "";
                                                                String symbol = asset.getSymbol() != null
                                                                                ? asset.getSymbol()
                                                                                : "";
                                                        ...
                                                        asset -> {
                                                                try {
                                                                        ...
                                                                        String symbol = asset.getSymbol() != null
                                                                                        ? asset.getSymbol()
                                                                                        : "";
                                                                        String snippet = createSnippet(name, symbol,
                                                                                        request.getQuery(), 150);

                                                                        BigDecimal currentValue = asset
                                                                        ...
                                                                        return SearchResultDto.builder()
                                                                                        ...
                                                                                        .id(asset.getId())
                                                                                        .title(name)
                                                                                        .subtitle(symbol + " - " + asset
                                                                                                        .getType()
                                                                                                        .name())
                                                                                        .amount(currentValue)
                                                                                        .currency(asset.getCurrency())
                                                                                        .date(asset.getPurchaseDate())
                                                                                        .icon(getAssetTypeIcon(asset
                                                                                                        .getType()))
                                                                                        .color("#8b5cf6")
                                                                                        .snippet(snippet)
                                                                        ...
                                                                        log.error("Error processing asset search result",
                                                                                        e);
                                                                        return null;
                                                                }
                                                        })
         ...
         *
         * <p>
         * Since description, notes, tags, and payee are encrypted, FTS5 cannot be used.
         * Fetches all user transactions via JPA (which decrypts transparently) and
         * filters in Java.
         *
        ...
        private List<SearchResultDto> searchTransactions(
                ...
                try {
                        ...
                        if (contextKey != null) {
                                ...
                                List<Long> matchingIds = searchTokenService.search(
                                                        ...
                                                        asset -> {
                                                                try {
                                                                        ...
                                                                        if (decryptedName.toLowerCase()
                                                                                ...
                                                                                BigDecimal value = asset.getQuantity()
                                        ...
                                        .map(
                                                        property -> {
                                                                try {
                                                                        ...
                                                                        log.error("Error decrypting real estate data",
                                                                                        e);
                                                                        return null;
                                                                }
                                                        })
                                        ...
                                        .map(
                                                        budget -> {
                                                                try {
                                                                        ...
                                                                        if ((notes != null && notes.toLowerCase()
                                                                                        .contains(lowerQuery))
                                        ...
                                        .map(
                                                        rt -> {
                                                                try {
                                                                        ...
                                                                        log.error("Error decrypting recurring transaction data",
                                                                                        e);
                                                                        return null;
                                                                }
                                                        })
        ...
        private boolean matchesTextQuery(org.openfinance.entity.Transaction t, String lowerQuery) {
                if (t.getDescription() != null && t.getDescription().toLowerCase().contains(lowerQuery))
                        return true;
                if (t.getNotes() != null && t.getNotes().toLowerCase().contains(lowerQuery))
                        return true;
                if (t.getPayee() != null && t.getPayee().toLowerCase().contains(lowerQuery))
                        return true;
                if (t.getTags() != null && t.getTags().toLowerCase().contains(lowerQuery))
                        return true;
                return false;
        }
        ...
        /**
         * Runs a supplier with the given encryption key set in EncryptionContext,
         * ensuring cleanup afterwards. Used to propagate the request-thread key to
