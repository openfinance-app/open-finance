...
public class TestDataSeeder implements CommandLineRunner {
    ...
    private void seedAccounts(Long userId, SecretKey encryptionKey) {
        ...
        for (int i = accounts.size(); i < base; i++) {
            ...
            accounts.add(
                    createAccount(
                            userId, name, type, balance, currency, encryptionKey, accNum, inst));
        }
    ...
    private Account createAccount(
        ...
        return Account.builder()
                ...
                .accountNumber(
                        accountNumber != null
                                ? encryptionService.encrypt(accountNumber, encryptionKey)
                                : null)
                .isActive(true)
                .createdAt(LocalDateTime.now())
                .build();
    ...
    private void seedLiabilities(Long userId, SecretKey encryptionKey) {
        ...
        for (int i = 0; i < base; i++) {
            ...
            l.setInterestRate(
                    encryptionService.encrypt(interestRateValue.toPlainString(), encryptionKey));
            ...
            l.setMinimumPayment(
                    encryptionService.encrypt(minimumPayment.toPlainString(), encryptionKey));
        ...
        for (int i = 0; i < base; i++) {
            RealEstateProperty p =
                    RealEstateProperty.builder()
                            ...
                            .address(
                                    encryptionService.encrypt(
                                            (100 + i) + " Demo St, City", encryptionKey))
                            ...
                            .currentValue(
                                    encryptionService.encrypt(
                                            String.valueOf(120000 + rnd.nextInt(900000)),
                                            encryptionKey))
                            .currency("EUR")
                            ...
                            .notes(
                                    encryptionService.encrypt(
                                            "Seeded property notes", encryptionKey))
                            .isActive(true)
                            .build();
            list.add(p);
        }
